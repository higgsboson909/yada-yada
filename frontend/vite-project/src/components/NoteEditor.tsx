...
export function NoteEditor({ note, onDone, onDeleted }: NoteEditorProps) {
  ...
  return (
    <form className="mx-auto flex min-h-[65vh] max-w-[760px] flex-col" onSubmit={handleSubmit((form) => saveNote.mutate(form))}>
      ...
      <input id="note-title" className="bg-transparent font-serif text-[clamp(34px,5vw,52px)] font-bold text-stone-800 outline-none" {...register('title')} placeholder="Untitled note" aria-invalid={Boolean(errors.title)} aria-describedby={errors.title ? 'note-title-error' : undefined} />
      ...
      <textarea id="note-content" className="mt-6 min-h-[420px] flex-1 resize-none bg-transparent font-serif text-lg leading-relaxed text-stone-700 outline-none" {...register('content')} placeholder="Start writing…" aria-invalid={Boolean(errors.content)} aria-describedby={errors.content ? 'note-content-error' : undefined} />
      ...
      <div className="mt-5 flex flex-wrap items-center gap-2 border-t border-stone-200 pt-5">
        {note && <button type="button" className="inline-flex min-h-11 items-center gap-2 rounded-xl px-4 py-3 text-red-700 transition hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50" onClick={requestDelete} disabled={pending}><Trash2 size={16} aria-hidden="true" /> Delete note</button>}
        <span className="flex-1" />
        <button type="button" className="min-h-11 rounded-xl px-4 py-3 text-stone-600 transition hover:bg-stone-200 disabled:cursor-not-allowed disabled:opacity-50" onClick={onDone} disabled={pending}>Cancel</button>
