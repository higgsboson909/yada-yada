...
export function Workspace({ onLoggedOut }: { onLoggedOut: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-stone-50">
      <header className="sticky top-0 z-10 flex h-[68px] items-center justify-between border-b border-stone-200 bg-white px-[18px] sm:px-7">
        ...
        <Link to="/app" className="font-serif text-[25px] font-bold text-stone-800 no-underline">yada yada<span className="text-amber-600">.</span></Link>
      ...
      <div className="grid min-h-[calc(100vh-68px)] md:grid-cols-[320px_1fr]">
        <aside id="workspace-navigation" className={`${navigationOpen ? 'block' : 'hidden'} border-b border-stone-200 bg-white p-6 md:block md:border-b-0 md:border-r md:px-4`} aria-label="Notes and checklists">
          <div className="flex items-center justify-between px-2 pb-2 text-[11px] font-extrabold tracking-[0.14em] text-stone-600 uppercase"><span>Notes</span><button className={miniButtonClass} onClick={startNote} aria-label="Create note"><Plus size={16} aria-hidden="true" /></button></div>
          ...
          {notes.data?.map((note: Note) => <button className={`grid min-h-11 w-full grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2.5 rounded-xl border-0 px-2.5 text-left text-stone-700 transition hover:bg-stone-100 ${selectedNoteId === note.id ? 'bg-stone-100 text-stone-900' : 'bg-transparent'}`} key={note.id} onClick={() => { setSelectedNoteId(note.id); setCreatingNote(false); setNavigationOpen(false) }}><FileText size={16} aria-hidden="true" /><span className="overflow-hidden text-ellipsis whitespace-nowrap">{note.title}</span></button>)}
          <div className="mb-2 mt-7 px-2 pb-2 text-[11px] font-extrabold tracking-[0.14em] text-stone-600 uppercase">Checklists</div>
          ...
          <form className="mt-2.5 grid grid-cols-[1fr_auto] gap-1.5 rounded-xl border border-dashed border-stone-400 p-2 pl-3" onSubmit={submitChecklist}>
            <label className="sr-only" htmlFor="new-checklist">New checklist name</label>
            <input id="new-checklist" className="w-full bg-transparent text-base text-stone-700 outline-none" value={newChecklist} onChange={(event) => setNewChecklist(event.target.value)} placeholder="New checklist" disabled={createChecklist.isPending} />
        ...
        <section className="bg-stone-50 p-5 sm:p-[clamp(24px,6vw,84px)]" aria-label="Note editor">
          {creatingNote || selectedNote ? <NoteEditor key={selectedNote?.id ?? 'new-note'} note={selectedNote} onDone={closeEditor} onDeleted={closeEditor} /> : <div className="flex min-h-[40vh] flex-col items-center justify-center text-center md:min-h-[55vh]"><div className="grid size-[58px] place-items-center rounded-[18px] bg-amber-100 text-amber-700"><FileText size={28} aria-hidden="true" /></div><h2 className="mb-2 mt-5 font-serif text-[30px] font-bold text-stone-800">Your thoughts, in one place.</h2><p className="text-stone-600">Choose a note or create a new one to begin.</p><button className={`${buttonClass} mt-6`} onClick={startNote}><Plus size={16} aria-hidden="true" /> New note</button></div>}
        </section>
      </div>
    </div>
  )
