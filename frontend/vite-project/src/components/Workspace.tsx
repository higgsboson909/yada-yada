...
export function Workspace({ onLoggedOut }: { onLoggedOut: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-canvas text-fg">
      <header className="sticky top-0 z-10 flex h-[68px] items-center justify-between border-b border-border bg-surface px-[18px] sm:px-7">
        ...
        <Link to="/app" className="font-serif text-[25px] font-semibold text-fg no-underline">yada yada<span className="text-accent">.</span></Link>
      ...
      <div className="grid min-h-[calc(100vh-68px)] md:grid-cols-[320px_1fr]">
        <aside id="workspace-navigation" className={`${navigationOpen ? 'block' : 'hidden'} border-b border-border bg-surface p-6 md:block md:border-b-0 md:border-r md:px-4`} aria-label="Notes and checklists">
          <div className={sectionHeadingClass}><span>Notes</span><button className={miniButtonClass} onClick={startNote} aria-label="Create note"><Plus size={16} aria-hidden="true" /></button></div>
          ...
          {notes.data?.map((note: Note) => <button className={`grid min-h-11 w-full grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2.5 rounded-xl border-0 px-2.5 text-left transition hover:bg-surface-muted ${selectedNoteId === note.id ? 'bg-surface-muted text-fg' : 'bg-transparent text-muted'}`} key={note.id} onClick={() => { setSelectedNoteId(note.id); setCreatingNote(false); setNavigationOpen(false) }}><FileText size={16} aria-hidden="true" /><span className="overflow-hidden text-ellipsis whitespace-nowrap">{note.title}</span></button>)}
          <div className={`${sectionHeadingClass} mt-7`}><span>Checklists</span></div>
          ...
          <form className="mt-2.5 grid grid-cols-[1fr_auto] gap-1.5 rounded-xl border border-dashed border-border-strong p-2 pl-3" onSubmit={submitChecklist}>
            <label className="sr-only" htmlFor="new-checklist">New checklist name</label>
            <input id="new-checklist" className="w-full bg-transparent text-base text-fg outline-none placeholder:text-subtle" value={newChecklist} onChange={(event) => setNewChecklist(event.target.value)} placeholder="New checklist" disabled={createChecklist.isPending} />
        ...
        <section className="bg-canvas p-5 sm:p-[clamp(24px,6vw,84px)]" aria-label="Note editor">
          {creatingNote || selectedNote ? (
            ...
            <div className="flex min-h-[40vh] flex-col items-center justify-center text-center md:min-h-[55vh]">
              ...
              <p className="text-muted">Choose a note or create a new one to begin.</p>
