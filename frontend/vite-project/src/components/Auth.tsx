...
export function Auth({ signup = false, onAuthenticated }: AuthProps) {
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-[radial-gradient(circle_at_20%_10%,#fef3c7_0,transparent_32%),#fafaf9] p-6">
      <div className="w-full max-w-[430px] rounded-[28px] border border-stone-200 bg-white/95 p-6 shadow-[0_24px_70px_rgba(41,37,36,0.08)] sm:p-[42px]">
        <div className="grid size-11 place-items-center rounded-[14px] bg-stone-800 font-extrabold text-white" aria-hidden="true">Y</div>
        <p className="mb-2 mt-7 text-[11px] font-extrabold tracking-[0.18em] text-amber-700">YADA YADA</p>
        <h1 className="font-serif text-[34px] font-bold leading-tight text-stone-800 sm:text-[40px]">
          {signup ? 'Make space for your thoughts.' : 'Welcome back.'}
        </h1>
        <p className="mb-7 mt-3 text-stone-600">
          {signup
        ...
        <form onSubmit={handleSubmit(submit)} className="grid gap-[18px]" noValidate>
          {signup && (
            <label htmlFor="name" className="grid gap-2 text-[13px] font-semibold text-stone-800">
              Name
              <input id="name" className={inputClass} {...register('name')} autoComplete="name" placeholder="Your name" />
            </label>
          )}
          <label htmlFor="email" className="grid gap-2 text-[13px] font-semibold text-stone-800">
            Email
            ...
            {errors.email && <small id="email-error" className="text-red-700">{String(errors.email.message)}</small>}
          </label>
          <label htmlFor="password" className="grid gap-2 text-[13px] font-semibold text-stone-800">
            Password
            ...
            {errors.password && <small id="password-error" className="text-red-700">{String(errors.password.message)}</small>}
          </label>
        ...
        <p className="mt-6 text-center text-[13px] text-stone-600">
          ...
          <Link className="font-bold text-amber-800" to={signup ? '/login' : '/signup'}>
