...
9. Treat the user as a learner building full-stack development skills. For every requested change, explain both how to implement it and why that approach is used. Include relevant reasoning across React, TypeScript, FastAPI, SQLModel, Pydantic, PostgreSQL, authentication, testing, infrastructure, and related tooling. Prefer teaching through the user's actual code and clearly call out trade-offs, conventions, and important concepts rather than only providing the final result.
